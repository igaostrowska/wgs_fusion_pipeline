...
input_options = list(
  ...
  make_option(c("--novoalign_bin"), default = "novoalign", help = "Path to novoalign binary (default = novoalign)"),
  make_option(c("--novoindex_bin"), default = "novoindex", help = "Path to novoindex binary (default = novoindex)")
)

parsed_args <- parse_args(OptionParser(option_list = input_options))
input_file <- parsed_args$input_file
ref_genome <- parsed_args$ref_genome
novoalign_bin <- parsed_args$novoalign_bin
novoindex_bin <- parsed_args$novoindex_bin


sam_fields <- c("QNAME","FLAG","RNAME","POS","MAPQ","CIGAR","MRNM","MPOS","ISIZE","SEQ","QUAL","OPT")
input <- read_tsv(input_file)

# Validate required input files/paths early to avoid cryptic downstream errors
missing_bams <- input %>%
  distinct(path) %>%
...
if (length(missing_bams) > 0) {
  stop(
    paste0(
      ...
      ". Check host->container bind mounts and the 'path' column in input table."
    )
  )
}

if (!file.exists(ref_genome)) {
  stop(paste0("Reference FASTA not found: ", ref_genome))
}

if (!file.exists(paste0(ref_genome, ".fai"))) {
  stop(paste0("Reference FASTA index not found: ", ref_genome, ".fai"))
}

ref_contigs <- read_tsv(
  paste0(ref_genome, ".fai"),
  col_names = FALSE,
  show_col_types = FALSE
) %>% 
  pull(X1)

ref_has_chr_prefix <- any(str_detect(ref_contigs, "^chr"))

normalize_chr_for_reference <- function(chr_vector, ref_has_chr_prefix) {
  chr_vector <- as.character(chr_vector)
  if (ref_has_chr_prefix) {
    ifelse(str_detect(chr_vector, "^chr"), chr_vector, paste0("chr", chr_vector))
  } else {
    str_replace(chr_vector, "^chr", "")
  }
}
...
if (nrow(disc_reads) == 0) {
  stop(
    "No discordant reads found. Check BAM content, chromosome naming (e.g. chr1 vs 1), and fusion search regions."
  )
}
...
disc_read_cluster_bed <- disc_read_clusters %>% 
  ...
  mutate(chr = normalize_chr_for_reference(chr, ref_has_chr_prefix))
...
system2(
  command = "bedtools",
  args = c("getfasta", "-fi", ref_genome, "-bed", "output/discordant_read_clusters.bed", "-nameOnly"),
  stdout = "output/discordant_read_cluster_sequences.fa"
)
...
list.files("output/discordant_read_cluster_sequences/") %>% 
  ...
  mutate(
    fasta_path = paste0("output/discordant_read_cluster_sequences/", value),
    index_path = paste0(fasta_path, ".ndx")
  ) %>% 
  pwalk(~system2(command = novoindex_bin, args = c(..3, ..2)))
...
write_lines(character(), "output/softclip_alignment.txt")
write_lines(character(), "output/softclip_alignment_log.txt")
...
list.files("output/discordant_read_cluster_sequences/") %>% 
  ...
  filter(!str_detect(value, "\\.ndx$")) %>% 
  mutate(
    softclip_path = paste0("output/softclip_fasta_files/disc_read_cluster_", value),
    index_path = paste0("output/discordant_read_cluster_sequences/", value, ".ndx"),
    cmd = paste(
      shQuote(novoalign_bin),
      "-f", shQuote(softclip_path),
      "-d", shQuote(index_path),
      "-r ALL -o SAM -h-1-1 -l6",
      ">> output/softclip_alignment.txt",
      "2>> output/softclip_alignment_log.txt"
    )
  ) %>% 
  pull(cmd) %>% 
  walk(system)

alignment_raw <- read_tsv(
  "output/softclip_alignment.txt",
  comment = "@",
  col_names = FALSE,
  show_col_types = FALSE
...
if (ncol(alignment_raw) < 11) {
  stop("Softclip alignment output has fewer than 11 SAM columns; check novoalign output/log.")
}

alignment <- alignment_raw %>% 
  select(1:11) %>% 
  setNames(c("QNAME","FLAG","RNAME","POS","MAPQ","CIGAR","MRNM","MPOS","ISIZE","SEQ","QUAL")) %>% 
  mutate(
    FLAG = as.numeric(FLAG),
    POS = as.numeric(POS),
    MAPQ = as.numeric(MAPQ),
    MPOS = as.numeric(MPOS),
    ISIZE = as.numeric(ISIZE)
  ) %>% 
  filter(MAPQ != 0) %>% 
